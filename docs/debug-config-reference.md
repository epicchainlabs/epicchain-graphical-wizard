...
generated by [EpicChainTrace](https://github.com/epicchainlabs/epicchain-trace-visualizer#EpicChainTrace) and EpicChainTraceVisualizer (via the --trace option).
...
Specifies the path to a EpicChainTraceVisualizer file. Specifying this property enables accounts in other launch
...
Specifies the path to a EpicChainTraceVisualizer checkpoint file. Specifying this property enables the debugger to 
use real blockchain data while debugging.

> Note, EpicChain Graphical Wizard *NEVER* updates an actual EpicChainTraceVisualizer node instance. Contract storage changes are
...
representative data stored in an actual instance of a EpicChainTraceVisualizer node.
...
* `account` specifies a wallet account, as a EpicChain address (such as `NViTnvofZYshnqppD6ksCtUzBDn54an5hM`),
  ...
  EpicChainTraceVisualizer wallet name such as `@alice`. The `@` prefixed syntax is only supported if the `epicchain-trace-visualizer`
